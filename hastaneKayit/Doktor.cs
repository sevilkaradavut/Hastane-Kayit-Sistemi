...
namespace HastaneKayit
{
    public partial class Doktor : Form
    {
        ...
        private void Doktor_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                while (rd.Read())
                {
                    ...
                    lblKullaniciAdi.Text = rd["DoktorAdiSoyadi"].ToString();
        ...
        void BilgileriGetir()
        {
            ...
            {
                frm1.uyelerBaglantisi.Open();
                OleDbCommand cmd = new OleDbCommand("Select * From DoktorBilgi where doktorAdiSoyadi='" + lblKullaniciAdi.Text + "'", frm1.uyelerBaglantisi);
                cmd.ExecuteNonQuery();
                OleDbDataReader rd = cmd.ExecuteReader();
                while (rd.Read())
                {
               hakkımda_txtbx.Text= rd["hakkimda"].ToString();
                }
                frm1.uyelerBaglantisi.Close();
            }
            catch (Exception hata)
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            string sorgu = "Update DoktorBilgi Set hakkimda=@hakkimda Where doktorAdiSoyadi='" + lblKullaniciAdi.Text + "'";
            

            komut = new OleDbCommand(sorgu, frm1.uyelerBaglantisi);
            komut.Parameters.AddWithValue("@hakkimda", hakkımda_txtbx.Text);
        ...
        private void kaydet_btn_Click(object sender, EventArgs e)
        {
            ...
            OleDbCommand cmd = new OleDbCommand("insert into DoktorBilgi(doktorAdiSoyadi,hakkimda) values ('" + lblKullaniciAdi.Text + "','" + hakkımda_txtbx.Text + "')", frm1.uyelerBaglantisi);
            cmd.ExecuteNonQuery();
